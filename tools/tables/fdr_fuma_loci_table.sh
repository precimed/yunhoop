...
if [ $# -ne 3 ] && [ $# -ne 5 ] && [ $# -ne 9 ]; then
  echo "Usage:     sh fdr_fuma_loci_table.sh fdr_clump_loci_file fdr_fuma_snp_table outfile [trait1locifile trait2locifile] [fdr_snp_table fuma_gwasc keywords1 keywords2]"
  ...
  echo "Example:   sh fdr_fuma_loci_table.sh conj.result.clump.loci.csv ADHD_vs_MOOD_snps.txt ADHD_vs_MOOD_loci.txt adhd_gwas_loci.csv mood_gwas_loci.csv conj.result.clump.snps.csv gwascatalog.txt 'attention & deficit & hyperact' 'mood & instab'"
  exit 0
fi
#-------------------------------------------------------------------------#
...
join -1 3 -2 3 -a 1 $fdr_clump_loci_file.sorted $fdr_fuma_snp_table.sorted | grep -v FDR | awk '{print $2,$3,$1,$4,$5,$6,$7,$16,$17,$23,$24,$25,$26,$27,$28,$29,$33,$34,$35,$36,$37,$38,$39,$40,$41}' OFS='\t' | sort -n -k1,1 >> $outfile
...
if [ $# -eq 9 ]; then
    ...
    if [ "$fdr_snp_table" != "-" ] && [ "$fuma_gwasc" != "-" ]; then
        ...
        if [ "$keywords1" != "-" ]; then
            ...
            sh $(dirname $0)/../novelty/check_loci_in_gwasc.sh $fdr_snp_table $fuma_gwasc "$keywords1" ${outfile%.*}_${tag1}_gwasc.txt
        ...
        if [ "$keywords2" != "-" ]; then
            ...
            sh $(dirname $0)/../novelty/check_loci_in_gwasc.sh $fdr_snp_table $fuma_gwasc "$keywords2" ${outfile%.*}_${tag2}_gwasc.txt
